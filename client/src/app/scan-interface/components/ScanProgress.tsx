...
export default function ScanProgress({ isScanning, onComplete, depth = 'basic', type = 'message' }: ScanProgressProps) {
  ...
  return (
    <div className="fixed inset-0 bg-background/90 backdrop-blur-md z-[100] flex items-center justify-center p-4 overflow-hidden">
      {/* Background Animated Circuits (SVG) */}
      ...
      <div className="relative bg-card rounded-3xl shadow-2xl p-6 md:p-10 max-w-2xl w-full border border-primary/20 overflow-hidden transform animate-in fade-in zoom-in duration-500">
        {/* Progress Bar Top */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="flex flex-col space-y-6">
             ...
             <div className="bg-slate-950/90 rounded-2xl p-5 font-accent text-[11px] text-blue-400 h-40 overflow-hidden border border-primary/20 shadow-2xl group ring-1 ring-white/5">
                <div className="flex items-center justify-between mb-3 pb-2 border-b border-white/10">
                   ...
                   <span className="text-white/30 text-[9px] uppercase font-bold tracking-[0.2em]">Engine Instance #842</span>
                </div>
                <div ref={scrollRef} className="space-y-1.5 h-24 overflow-y-auto no-scrollbar scroll-smooth">
                   {logs.map((log, i) => (
                      <div key={i} className="animate-in slide-in-from-left-2 duration-300 flex gap-2">
                         ...
                         <span className={log.startsWith('>') ? 'text-blue-200/80' : 'text-primary font-bold'}>{log}</span>
                      </div>
                   ))}
                   <div className="w-2 h-4 bg-primary/40 inline-block animate-pulse ml-0.5 align-middle" />
                </div>
             </div>
