...
export default function CompanyReportPage() {
  ...
  // Parse data from URL params (assuming we pass base64 or simpler params for now)
  // In a real app, we might fetch by ID, but hereby we'll decode the state passed 
  // or simply display what we have if we pass it via query.
  // For this demo, let's assume we pass the data parameters directly.
  ...
  const scanId = searchParams.get('scanId'); // ID specifically for Feedback tracking
  ...
  const submitFeedback = async (rating: number) => {
    if (!scanId) return;
    try {
      // Direct call to API (Assuming backend is on same host/proxy or absolute)
      // For this environment, we use relative /api/feedback if proxy is set up, or explicit localhost
