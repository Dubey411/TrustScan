...
def analyze_frequency_domain(image_path_or_bytes):
    try:
        ...
        ai_score = 0.0
        if hfer < 0.05: ai_score += 0.45
        elif hfer < 0.08: ai_score += 0.30
        elif hfer < 0.12: ai_score += 0.10
        if grid_spike_ratio > 50: ai_score += 0.35
        elif grid_spike_ratio > 25: ai_score += 0.20
        elif grid_spike_ratio > 15: ai_score += 0.08
        if abs(corr) < 0.3: ai_score += 0.20
        elif abs(corr) < 0.6: ai_score += 0.08
        ai_score = float(min(1.0, ai_score))
        is_ai_generated = ai_score > 0.40
        generator_hint = "Unknown / Real Photo"
        if is_ai_generated:
            if hfer < 0.04 and grid_spike_ratio > 40:
                generator_hint = "Latent Diffusion Model (SD / SDXL / FLUX / DALL-E 3)"
            elif hfer < 0.06:
                generator_hint = "GAN-based Generator (Midjourney / StyleGAN / Imagen)"
            else:
                generator_hint = "AI-Assisted / Partial Generation (Inpainting / Canva AI)"
...
def analyze_dct_uniformity(image_path_or_bytes):
    try:
        ...
        if std_ac > 0:
            ...
            kurtosis = 0.0
        is_gaussian_like = kurtosis < 1.5
        dct_ai_score = max(0.0, min(1.0, (2.0 - kurtosis) / 4.0)) if kurtosis < 2.0 else 0.0
...
def run_full_forensics(image_path):
    ...
    ai_gen_score = 0.0
    if exif.get("has_ai_signature"): ai_gen_score += 0.70
    ai_gen_score += fft.get("ai_generation_score", 0.0) * 0.50
    ai_gen_score += dct.get("dct_ai_score", 0.0) * 0.20
    if not noise.get("has_noise_anomaly") and ela.get("mean_ela", 99) < 10: ai_gen_score += 0.10
    ai_gen_score = float(min(1.0, ai_gen_score))
    is_ai_generated = ai_gen_score > 0.45
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        # Self-test: generate a synthetic image and pass as raw bytes
        dummy = Image.new('RGB', (300, 300), color=(128, 90, 45))
        buf = io.BytesIO()
        dummy.save(buf, 'JPEG', quality=95)
        raw_bytes = buf.getvalue()  # Pass bytes, not BytesIO
